...
import com.ntt.crfactory.repository.WaveRepository;
import com.ntt.crfactory.service.WaveService;
import com.ntt.crfactory.service.dto.WaveDTO;
import com.ntt.crfactory.service.mapper.WaveMapper;
...
public class WaveResourceIT {
    ...
    @Mock
    private WaveRepository waveRepositoryMock;

    @Autowired
    private WaveMapper waveMapper;

    @Mock
    private WaveService waveServiceMock;
    ...
    public void getAllWavesWithEagerRelationshipsIsEnabled() throws Exception {
        when(waveServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restWaveMockMvc.perform(get("/api/waves?eagerload=true"))
            .andExpect(status().isOk());

        verify(waveServiceMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({"unchecked"})
    public void getAllWavesWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(waveServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));
