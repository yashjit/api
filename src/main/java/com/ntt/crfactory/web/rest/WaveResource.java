...
import com.ntt.crfactory.web.rest.errors.BadRequestAlertException;
...
/**
 * REST controller for managing {@link com.ntt.crfactory.domain.Wave}.
 */
@RestController
@RequestMapping("/api")
public class WaveResource {
    ...
    private static final String ENTITY_NAME = "apiWave";
     ...
     */
    @GetMapping("/waves")
    public ResponseEntity<List<WaveDTO>> getAllWaves(Pageable pageable, @RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        log.debug("REST request to get a page of Waves");
        Page<WaveDTO> page;
        if (eagerload) {
            page = waveService.findAllWithEagerRelationships(pageable);
        } else {
            page = waveService.findAll(pageable);
        }
